...
def play_music(song_name):
    """Play a specific song in Music app using AppleScript"""
    try:
        # AppleScript to search and play a song in Music app
        script = f'''
        tell application "Music"
            if not (exists playlist "Library") then
                reopen
            end if
            activate
            play (every track of playlist "Library" whose name contains "{song_name}")
        end tell
        '''
        subprocess.run(['osascript', '-e', script], check=True)
        ...
        print(f"❌ Failed to play '{song_name}'. Make sure the song exists in your library.")
        return False
    except FileNotFoundError:
        print("❌ 'osascript' command not found. This script is designed for macOS.")
        return False
